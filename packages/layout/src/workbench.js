...
export function createWorkbench(root, adapter, options = {}) {
    const doc = root.ownerDocument;
    const win = doc.defaultView;
    const themes = options.themes?.length ? options.themes : builtinThemes;
    ...
    let theme = themes.find(item => item.id === (options.theme ?? savedTheme)) ??
            ...
            adapter.inspect(current).forEach(field => {
                ...
                input.onchange = () => {
                    ...
                    if (destroyed || fieldRevision !== revision || selection?.id !== objectId ||
                        !latest || !adapter.update || input.disabled)
                        return;
    ...
    function objectAt(range) {
        ...
        const size = (object) => object.range.end - object.range.start;
        if (containing.length)
            return containing.sort((a, b) => size(a) - size(b))[0];
        const distance = (object) => Math.max(object.range.start - range.start, range.start - object.range.end, 0);
        return objects.sort((a, b) => distance(a) - distance(b) || size(a) - size(b))[0] ?? null;
    }
    function reveal(range) {
        if (destroyed || !validRange(range))
            return;
    ...
    function edit(edits) {
        if (destroyed)
            return;
        try {
            changeSource(applyEdits(source, edits));
        }
        catch (error) {
            report(error);
        }
    ...
    async function render() {
        ...
        try {
            const result = await adapter.render({
                ...
                select: id => { if (isCurrent())
                    select(id); },
                reveal: range => { if (isCurrent())
                    reveal(range); },
                edit: edits => { if (isCurrent())
                    edit(edits); },
            });
    ...
    async function parse() {
        ...
        try {
            ...
            if (destroyed || revision !== currentRevision || controller.signal.aborted)
                return;
    ...
    function changeSource(value, history = true) {
        if (destroyed || value === source)
            return;
        if (history) {
            undo.push(source);
            redo.length = 0;
    ...
    function undoSource() {
        if (!undo.length || destroyed)
            return;
        redo.push(source);
        changeSource(undo.pop(), false);
    }
    function redoSource() {
        if (!redo.length || destroyed)
            return;
        undo.push(source);
        changeSource(redo.pop(), false);
    }
    function download(data, extension, mimeType) {
    ...
    button('New', actions, () => {
        if (win.confirm('Create a new document? This replaces the current source.'))
            changeSource('');
    });
    button('Reset', actions, () => {
        if (win.confirm('Reset to the example? This replaces the current source.'))
            changeSource(adapter.initialSource);
    });
    button('Import', actions, () => fileInput.click());
    button('Save source', actions, () => {
        try {
            download(source, adapter.extension, 'text/plain;charset=utf-8');
        }
        catch (error) {
            report(error);
        }
    ...
    adapter.commands?.forEach(command => {
        const node = button(command.label, actions, () => {
            ...
            if (!current || destroyed)
                return;
            try {
                changeSource(applyEdits(source, command.run(current)));
            }
            catch (error) {
                report(error);
            }
    ...
    listen(fileInput, 'change', () => {
        ...
        importReader = reader;
        reader.onload = () => {
            if (!destroyed && importRevision === revision && typeof reader.result === 'string')
                changeSource(reader.result);
        };
        reader.onerror = () => { if (!destroyed)
            report('Unable to read this file.'); };
        reader.onloadend = () => { if (importReader === reader)
            importReader = null; };
    ...
    editor = (options.createEditor ?? createTextareaEditor)({
        ...
        onSelect: range => {
            if (parsed && !destroyed && validRange(range))
                select(objectAt(range)?.id ?? null, false);
        },
    });
    void parse();
